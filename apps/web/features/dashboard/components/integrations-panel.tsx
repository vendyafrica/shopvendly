...
export function IntegrationsPanel({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      await linkInstagram({
        callbackURL: `/dashboard/${bootstrap.storeSlug}?connected=true`,
      });
    } finally {
    ...
    try {
      ...
      if (maybeLinkTikTok) {
        await maybeLinkTikTok({
          callbackURL: `/dashboard/${bootstrap.storeSlug}?tiktokConnected=true`,
              ...
              {showDangerZone && (
                <div className="px-6 pb-5 space-y-3">
                  <p className="text-xs text-muted-foreground">
                    Disconnects your Instagram account and removes all stored
                    tokens. <strong>Imported products are not deleted.</strong>
                  </p>
                  <Button
                    variant="destructive"
                    size="sm"
