    ...
    <div className="flex flex-col pb-20 w-full max-w-full overflow-hidden sm:hidden bg-slate-50/50 min-h-screen font-sans">
      ...
      <div className="grid grid-cols-2 gap-3 px-3 py-4">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className="flex flex-col overflow-hidden rounded-md border border-border/40 bg-white shadow-sm"
          >
...
interface ProductsMobileViewProps {
  ...
  onStatusChange?: (
    productId: string,
    newStatus: ProductTableRow["status"],
  ) => void;
...
export function ProductsMobileView({
  ...
  const isReadOnly = Boolean(
    bootstrap?.storeSlug === "vendly" && !bootstrap?.canWrite,
  );
  ...
  const AdminHref = bootstrap?.storeSlug
    ? `/admin/${bootstrap.storeSlug}`
    : "/admin";
  ...
  const publishProducts = async (items: ProductTableRow[]) => {
    ...
    await Promise.all(
      items.map((product) => onStatusChange(product.id, "active")),
    );
    clearSelection();
  };
    ...
    <div className="-mx-4 -mt-4 flex flex-col w-[calc(100%+2rem)] sm:hidden bg-white min-h-screen font-sans">
      ...
      <div className="px-1 pt-6 pb-2 border-b border-slate-100 italic-style">
        ...
        <div className="space-y-1 mb-6 px-5">
          <h1 className="font-extrabold text-[16px] tracking-tight text-slate-900">
            {storeName}
          </h1>
          <p className="text-[14px] text-slate-600 leading-[1.5] max-w-[95%]">
            {bootstrap?.storeDescription ||
              "Manage your product catalog and track sales performance in real-time."}
          </p>
          <Link
            href={
              bootstrap?.storeSlug
                ? `https://shopvendly.store/${bootstrap.storeSlug}`
                : "#"
            }
            target="_blank"
          ...
          >
            {bootstrap?.storeSlug
              ? `shopvendly.store/${bootstrap.storeSlug}`
              : "shopvendly.store"}
        ...
        <div className="flex gap-2.5 px-5 mb-6">
          <Button
            ...
            className={cn(
              ...
              isSelectionMode
                ...
                : "bg-slate-100 border-none text-slate-900 hover:bg-slate-200",
            )}
            onClick={() => {
              if (isReadOnly) return;
          ...
          <Button
            size="sm"
            className={cn(
              "flex-1 h-10 font-bold text-[13px] rounded-lg shadow-none border-none transition-all active:scale-[0.97]",
              isSelectionMode
            ...
            onClick={() => {
              if (isReadOnly) return;
              if (isSelectionMode) {
                void publishProducts(selectedProducts);
                return;
              }
              handleAddProduct();
            }}
          ...
          >
            <HugeiconsIcon
              icon={isSelectionMode ? Tick01Icon : ShoppingBag01Icon}
              className="size-4"
            />
            {isSelectionMode
              ? isPublishing
      ...
      <div className="flex flex-col gap-3 px-2 py-6 pb-20">
        {filteredRows.length === 0 ? (
          <div className="py-24 flex flex-col items-center justify-center text-center opacity-80 rounded-[32px] border-2 border-dashed border-border/40 bg-muted/5">
            ...
            <p className="text-[13px] text-muted-foreground mt-1 px-8 text-balance">
          ...
          filteredRows.map((product) => (
            <div
              ...
              className={cn(
                ...
                (isSelectionMode && product.status === "active") || isReadOnly
                  ? "opacity-40"
                  : "",
                isSelectionMode &&
              ...
              onClick={() => {
                ...
                if (isSelectionMode && product.status !== "active")
