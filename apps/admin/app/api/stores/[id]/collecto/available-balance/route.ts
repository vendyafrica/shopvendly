...
export async function GET(
    ...
    for (const baseUrl of apiBaseCandidates) {
        try {
            const res = await fetch(`${baseUrl}/api/stores/${encodeURIComponent(id)}/collecto/available-balance`, {
                cache: "no-store",
            });
            const json = await res.json().catch(() => ({}));
            return NextResponse.json(json, { status: res.status });
        } catch (error) {
            console.error("Collecto available balance proxy failed", { baseUrl, id, error });
        }
    }

    return NextResponse.json({ error: "Could not reach API server for Collecto balance." }, { status: 502 });
}
